...
// Clean out existing files in Downloads\QR Code
...
async function createUltraHDQRCard(item) {
    ...
    console.log(`Generated Ultra-HD Simple QR (${item.num}): ${numPath}`);
}

async function run() {
    console.log('Generating Ultra-HD 4K Resolution Print-Ready QR Codes (3200x4000px)...');
    for (const item of filteredPlaces) {
        await createUltraHDQRCard(item);
    }
    console.log(`\nAll ${filteredPlaces.length} Ultra-HD QR Code files generated successfully in: ${targetDir}`);
}

run().catch(err => {
    console.error('Error generating Ultra-HD QR codes:', err);
    process.exit(1);
});
