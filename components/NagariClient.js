...
export default function NagariClient() {
    ...
    const ATTRACTIONS = [
        { id: "shiva", icon: <Flower2 size={20} /> },
        { id: "hathiyon", icon: <Columns size={20} /> },
        { id: "prakash", icon: <Sun size={20} /> }
    ];
    ...
    const handleAudioPlay = (sectionId, customText = null) => {
        ...
        const textToSpeak = customText || `${t(`nagari.attractions.${sectionId}.name`)}. ${t(`nagari.attractions.${sectionId}.desc`)}`;
            ...
            <main className="fort-main">
                ...
                >
                    <div className="section-header">
                        <h2 className="section-title text-gold">{t("nagari.section.overview")}</h2>
                        <div className="title-divider"></div>
                    </div>
                    <div className="overview-grid">
                        <div className="overview-text">
                            <p className="lead-para">{t("nagari.overview.p1")}</p>
                            <p>{t("nagari.overview.p2")}</p>
                            
                            <button 
                                className={`audio-btn ${playingAudio === 'overview' ? 'playing' : ''}`}
                                onClick={() => handleAudioPlay('overview', `${t("nagari.overview.p1")} ${t("nagari.overview.p2")}`)}
                ...
                >
                    <div className="section-header">
                        <h2 className="section-title text-gold">{t("nagari.section.sibi")}</h2>
                        <div className="title-divider"></div>
                    </div>
                    <div className="overview-grid">
                        <div className="overview-text">
                            <p>{t("nagari.sibi.p1")}</p>
                            <p>{t("nagari.sibi.p2")}</p>
                            <button 
                                className={`audio-btn ${playingAudio === 'sibi' ? 'playing' : ''}`}
                                onClick={() => handleAudioPlay('sibi', `${t("nagari.sibi.p1")} ${t("nagari.sibi.p2")}`)}
                                style={{ marginTop: '2rem' }}
                            >
                                {playingAudio === 'sibi' ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" />}
                                <span>{playingAudio === 'sibi' ? t("fort.audio.playing") : t("fort.audio.listen")}</span>
                            </button>
                        </div>
                    </div>
                </motion.section>
                ...
                {/* ═══ ATTRACTIONS ══════════════════════════ */}
                <section id="attractions" className="fort-section">
                    <motion.div 
                    ...
                    >
                        <h2 className="section-title text-gold">{t("nagari.section.attractions")}</h2>
                        <div className="title-divider"></div>
                    </motion.div>
                            ...
                            >
                                <div className="mon-content">
                                    <h3 className="mon-name" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                        ...
                                        {t(`nagari.attractions.${m.id}.name`)}
                                    </h3>
                                    <p className={`mon-desc ${expandedSections[m.id] ? 'expanded' : ''}`}>
                                        {t(`nagari.attractions.${m.id}.desc`)}
                                    </p>
                ...
                >
                    <div className="section-header">
                        <h2 className="section-title text-gold">{t("nagari.references.title")}</h2>
                        <div className="title-divider"></div>
                    </div>
                    <div className="premium-glass" style={{ padding: '2rem', borderRadius: '16px' }}>
                        <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                            <li style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
                                ...
                                <div>
                                    <h3 style={{ fontSize: '1.1rem', marginBottom: '0.4rem', color: '#fff', fontFamily: 'var(--ff-sans)', fontWeight: '600' }}>{t("nagari.references.official")}</h3>
                                    <a 
                                        href={t("nagari.references.official_url")} 
                                        target="_blank" 
                                        rel="noopener noreferrer" 
                                        style={{ color: 'var(--gold)', textDecoration: 'none', fontSize: '0.95rem', display: 'inline-flex', alignItems: 'center', gap: '4px' }}
                                    >
                                        {t("nagari.references.official")} <span>→</span>
                                    </a>
                                </div>
                            </li>
                        </ul>
